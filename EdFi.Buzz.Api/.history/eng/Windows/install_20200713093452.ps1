...
function Install-NodeService {
  ...
  $buzzApiExe = "$InstallPath\$winSwVersion\BuzzApi.exe"
  if ((Test-Path -Path "$InstallPath\$winSwVersion\WindowsService.exe")) {
    Move-Item -Path $winServiceExe -Destination $buzzApiExe
  }

  # Copy the config XML file to install directory
  $xmlFile = "$InstallPath\$winSwVersion\BuzzApi.xml"
  Copy-Item -Path "$PSScriptRoot\BuzzApi.xml" -Destination $xmlFile -Force
...
Write-Host "Begin Buzz API installation..." -ForegroundColor Yellow
...
Write-Host "End Buzz API installation." -ForegroundColor Yellow
