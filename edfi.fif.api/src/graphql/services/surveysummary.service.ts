...
constructor(
    ...
    @InjectRepository(SurveySummaryQuestionsEntity) private readonly FixItFridayQuestionsRepository: Repository<SurveySummaryQuestionsEntity>
  ) {}
  ...
  async findAll(title: string, sectionkey: string): Promise<SurveySummaryEntity[]> {
    if (title)
    {
      ...
      .createQueryBuilder("SurveySummary")
      .where(`SurveySummary.sectionkey = '${sectionkey}' AND LOWER(SurveySummary.title) like LOWER('%${title}%')`)
      .getMany();
    }
    else
      return this.FixItFridayRepository.find({ where: { sectionkey: sectionkey } });
  }
  ...
  async findQuestionsBySurvey(surveykey: number): Promise<SurveySummaryQuestionsEntity[]> {
    return this.FixItFridayQuestionsRepository
      .createQueryBuilder('SurveySummaryQuestions')
      .leftJoin(
        SurveySummaryEntity,
        'ss',
        `SurveySummaryQuestions.surveykey = ss.surveykey and ss.surveykey='${surveykey}'`,
      )
