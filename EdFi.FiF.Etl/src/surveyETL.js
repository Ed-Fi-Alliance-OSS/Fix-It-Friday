...
async function getQuestions(surveykey, db) {
  return db.query('SELECT * FROM fif.surveyquestion sq where sq.surveykey = $1', [surveykey])
...
async function getOrSaveQuestions(questions, surveykey, db) {
  ...
  for (let i = 0; i < toSave.length; i += 1) {
    insertPromises.push(db.query('INSERT INTO fif.surveyquestion (surveykey, question) VALUES($1, $2);', [surveykey, toSave[i]]));
  }
